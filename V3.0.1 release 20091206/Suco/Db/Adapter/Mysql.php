...
class Suco_Db_Adapter_Mysql extends Suco_Db_Adapter_Abstract
{
	 ...
	 * @var array
	 */
	protected $_numericDataTypes = array('int', 'mediumint', 'smallint', 'bigint', 'tinyint', 'double', 'decimal');
	...
	public function execute($sql, $return = true)
	{
		...
		if (mysql_errno($this->_linkId)) {
			require_once 'Suco/Db/Exception.php';
			throw new Suco_Db_Exception("SQL执行失败. [".mysql_error()."]", 1002);
		}
	 ...
	 * @return string
	 */
	public function addSymbol($value, $prefix = null)
	{
		if (false !== strpos($value, '(') || false !== strpos($value, 'AS') 
			|| false !== strpos($value, 'as') 
			|| false !== strpos($value, '.') 
			|| false !== strpos($value, '`')) {
			return $value;
		}
