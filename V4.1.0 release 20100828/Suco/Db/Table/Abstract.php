...
class Suco_Db_Table_Abstract 
{
	...
	{
		$identity = $this->getIdentity();
		$cond = $this->getAdapter()->quoteInto("{$identity} = ?", $id);
	...
	public function filter($data)
	{
		...
		foreach ($metadata as $col) {
			...
			if (isset($columns[$name]) && $this->getAdapter()->isNumeric($col['TYPE'])) {
				$columns[$name] = floatval($columns[$name]);
			} else {
				$columns[$name] = strval($columns[$name]);
			}
			
			//强制转换数组
			if (is_array($data[$name])) {
				foreach ($data[$name] as $key => $val) {
					if (!$val) unset($data[$name][$key]);
				}
				$columns[$name] = implode($this->_arraySeparator, $data[$name]);
			}
			
			//强制截取
			if (isset($columns[$name]) && isset($col['LENGTH']) && strlen($columns[$name]) > $col['LENGTH']) {
				$columns[$name] = substr($columns[$name], 0, $col['LENGTH']);
			}
	...
	public function getUniqueCode($column = '', $length = 8)
	{
		$metadata = $this->getMetadata();
		$result = $this->select('MAX('.($column ? $column : $this->_identity).') AS result')->fetchCol('result') + 1;
		if (strlen($result) < $length) {
			$s = $length - strlen($result);
			$z = str_repeat('0', $s);
			$result = date('Ymd') . $z . $result;
